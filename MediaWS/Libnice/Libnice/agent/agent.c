...
{
	if (agent && agent->callbacks && agent->callbacks->lock)
		agent->callbacks->lock(agent, agent->callbacks->context);
...
//  g_static_rec_mutex_unlock (&agent_mutex);
	if (agent && agent->callbacks && agent->callbacks->unlock)
		agent->callbacks->unlock(agent, agent->callbacks->context);
...
{
  nice_rng_generate_bytes (agent->rng, sizeof(agent->tie_breaker), (gchar*)&agent->tie_breaker);
}

static void
nice_agent_init (NiceAgent *agent, int controlling_mode)
{
  ...
  agent->controlling_mode = controlling_mode > 0 ? TRUE : FALSE;
  agent->max_conn_checks = NICE_AGENT_MAX_CONNECTIVITY_CHECKS_DEFAULT;
  
	agent->timer_ta = NICE_AGENT_TIMER_TA_DEFAULT;
...
nice_agent_new (GMainContextRef ctx, NiceCompatibility compat, int controlling_mode)
{
	NiceAgent* agent = calloc(1, sizeof(NiceAgent)); // I like to have zero-fill
	nice_agent_init(agent, controlling_mode);
...
//{
//	NiceAgent* agent = calloc(sizeof(NiceAgent), 1); // I like to have zero-fill
//	nice_agent_init(agent);
//	agent->compatibility = compat;
//	agent->reliable = TRUE;
//	agent->main_context = ctx;
//
...
NICEAPI_EXPORT void nice_agent_set_signals(NiceAgent* agent, CompatCallback* callbacks)
{
...
//      stream->id, component->id);*/
//	if (agent->callbacks && agent->callbacks->cb_reliable_transport_writable)
//		agent->callbacks->cb_reliable_transport_writable(agent, stream->id, component->id);
//}
...
//  agent_lock();
//
//  if (g_current_main_current_source_is_destroyed()) {
  ...
  for (i = agent->streams; i; i = i->next) {
    ...
    if (stream->gathering) {
      ...
      //g_signal_emit (agent, signals[SIGNAL_CANDIDATE_GATHERING_DONE], 0, stream->id);
		if (agent->callbacks && agent->callbacks->cb_candidate_gathering_done)
			agent->callbacks->cb_candidate_gathering_done(agent, agent->callbacks->context, stream->id);
    }
  }
}
...
void agent_signal_initial_binding_request_received (NiceAgent *agent, Stream *stream)
{
  if (stream->initial_binding_request_received != TRUE) {
    ...
    //g_signal_emit (agent, signals[SIGNAL_INITIAL_BINDING_REQUEST_RECEIVED], 0, stream->id);
	  if (agent->callbacks && agent->callbacks->cb_initial_binding_request_received)
		  agent->callbacks->cb_initial_binding_request_received(agent, agent->callbacks->context, stream->id);
  }
}
...
//  }
  ...
  /*g_signal_emit (agent, signals[SIGNAL_NEW_SELECTED_PAIR], 0,
	...
	if (agent->callbacks && agent->callbacks->cb_new_selected_pair)
		agent->callbacks->cb_new_selected_pair(agent, agent->callbacks->context, stream_id, component_id, lf_copy, rf_copy);
...
void agent_signal_new_candidate (NiceAgent *agent, NiceCandidate *candidate)
{
  /*g_signal_emit (agent, signals[SIGNAL_NEW_CANDIDATE], 0,
	...
	if (agent->callbacks && agent->callbacks->cb_new_candidate)
		agent->callbacks->cb_new_candidate(agent, agent->callbacks->context, candidate->stream_id, candidate->component_id, candidate->foundation);
}
...
/*  g_signal_emit (agent, signals[SIGNAL_NEW_REMOTE_CANDIDATE], 0, 
		 ...
		 candidate->component_id, 
		 candidate->foundation);*/
	if (agent->callbacks && agent->callbacks->cb_new_remote_candidate)
		agent->callbacks->cb_new_remote_candidate(agent, agent->callbacks->context, candidate->stream_id, candidate->component_id, candidate->foundation);
}
...
//  }
  ...
  if (component->state != state && state < NICE_COMPONENT_STATE_LAST) {
    ...
    /*g_signal_emit (agent, signals[SIGNAL_COMPONENT_STATE_CHANGED], 0,
	  ...
	  if (agent->callbacks && agent->callbacks->cb_component_state_changed)
		  agent->callbacks->cb_component_state_changed(agent, agent->callbacks->context, stream_id, component_id, state);
  }
}
									   ...
									   Stream *stream, guint component_id)
{
	...
	{
		printf("WARNING: TURN/TCP is not supported");
...
NICEAPI_EXPORT gboolean nice_agent_set_stun(NiceAgent* agent, const gchar* stun_ip, guint port)
{
	g_return_val_if_fail (stun_ip, FALSE);
	g_return_val_if_fail (port, FALSE);
	agent_lock(agent);
	agent->stun_server_ip = g_strdup(stun_ip);
	agent->stun_server_port = port;
  ...
  guint stream_id)
{
  ...
  if (agent->local_addresses == NULL) {
    ...
    g_list_foreach (addresses, (GFunc) free, NULL);
...
void nice_agent_dispose (NiceAgent *agent)
{
	GSList *i;
	...
	//That's it, no reference check, no nothing.
	free(agent);
...
// We are going to just compatibly redefine the callback signature for darwin:
#ifndef DARWIN_COMPAT
						GIOChannelRef io,
						G_GNUC_UNUSED
...
//	} 
	...
	if (len > 0 && component->g_source_io_cb) 
	{
		...
		GSourceRef source = ctx->source;
		
		nice_debug ("Agent %p: _nice_agent_recv returned %d, errno (%d) : %s",
					agent, len, errno, strerror (errno));
    ...
    NiceSocket *socket)
{
	...
	if (!component->ctx)
		return;
	// channel and source are unknown yet in ctx:
	ctx = io_ctx_new(agent, stream, component, socket, NULL, NULL);
#ifndef DARWIN_COMPAT
  channel = g_io_channel_unix_new (socket->fileno);
  ...
  g_source_set_callback (source, (GSourceFunc) nice_agent_g_source_cb, ctx, (GDestroyNotify) io_ctx_free);
  nice_debug ("Agent %p : Attach source %p (stream %u).", agent, source, stream->id);
  g_source_attach (source, component->ctx);
#else
	channel = darwin_socket(socket->fileno, G_IO_IN | G_IO_ERR, nice_agent_g_source_cb, ctx, (GDestroyNotify) io_ctx_free);
	source = darwin_add_run_loop_socket_source(channel, component->ctx);
	nice_debug ("Agent %p : Attach source %p (stream %u).", agent, source, stream->id);
#endif
	ctx->channel = channel;
	ctx->source = source;
...
GSourceRef agent_timeout_add_with_context (NiceAgent *agent, guint interval,
...
#ifdef DARWIN_COMPAT
	source = darwin_timeout(interval, agent->main_context, function, data);
#else
...
#else
	  char* buf = malloc(strlen(software) + strlen(PACKAGE_STRING) + 10);
	  sprintf(buf, "%s/%s", software, PACKAGE_STRING);
	  agent->software_attribute = buf;
#endif
  }
