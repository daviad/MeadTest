...
+ (BOOL)isGifImage:(NSURL *)imageURL
{
    FILE* src = fopen([[imageURL path] cStringUsingEncoding:NSUTF8StringEncoding], "r");
	if (NULL == src) {
        RCError(@"load file %@ error",[imageURL path]);
		return NO;
	}
	char buf[4];
    fread(&buf, sizeof(char), 4, src);
	fclose(src);
