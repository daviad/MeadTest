...
/////////////////////////////////////////////////////////////////////////////////////////////////////
//+ (TTStyledText*)textFromLoochaText:(NSString*)source {
//    return [self textFromLoochaText:source lineBreaks:NO URLs:YES];
//}
//
//
/////////////////////////////////////////////////////////////////////////////////////////////////////
//+ (TTStyledText*)textFromLoochaText:(NSString*)source lineBreaks:(BOOL)lineBreaks URLs:(BOOL)URLs {
//    TTStyledTextParser* parser = [[[TTStyledTextParser alloc] init] autorelease];
//    parser.parseLineBreaks = lineBreaks;
//    parser.parseURLs = URLs;
//    [parser parseLoochaText:source];
//    if (parser.rootNode) {
//        return [[[TTStyledText alloc] initWithNode:parser.rootNode] autorelease];
//        
//    } else {
...
- (void)layoutFrames {
  ...
  TTStyledLayout* layout = [[TTStyledLayout alloc] initWithRootNode:_rootNode];
    layout.whichLable = self.whichLable;
  layout.width = _width;
  layout.font = _font;
  layout.textAlignment = _textAlignment;
  if(maxnumOfLines)
      layout.maxLines = maxnumOfLines;
...
-(CGFloat)calculateFirstLineWith
{
    ...
    firstLineWith = sumWith;
    return sumWith;
}
//-----

-(CGFloat)getContentViewMaxHeight
{
    //    CGFloat sumH = 0;
    //      CGFloat sumWith = 0;
    //    TTStyledFrame* frame = self.rootFrame;
...
- (void)drawAtPoint:(CGPoint)point highlighted:(BOOL)highlighted {
  ...
  CGContextTranslateCTM(ctx, point.x, point.y);
    ...
    else 
    {
        ...
        {
            [frame drawInRect:frame.bounds];
            firstLineWith +=frame.width;
            frame = frame.nextFrame;
        }
        
        if (firstLineWith>maxWith)
        {
            firstLineWith = maxWith;
        }
    }
