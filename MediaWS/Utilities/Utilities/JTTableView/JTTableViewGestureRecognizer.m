...
CGFloat const JTTableViewCommitEditingRowDefaultLength = 50;
...
- (void)panGestureRecognizer:(UIPanGestureRecognizer *)recognizer {
    if ((recognizer.state == UIGestureRecognizerStateBegan
        ...
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:[self.addingIndexPath row] inSection:[self.addingIndexPath section]];
        ...
        if (fabsf(translation.x) >= commitEditingLength && self.addingCellState != JTTableViewCellEditingStateRight) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer == self.panRecognizer) {
        ...
        if(!indexPath)
            return NO;
        if([self.delegate respondsToSelector:@selector(shouldBeginPanGestureRecognizerAtIndex:)])
        {
            if(![self.delegate shouldBeginPanGestureRecognizerAtIndex:indexPath])
            return NO;
        }
        ...
        // pan gesture.
        // 禁止右滑
        if (fabsf(point.y) > fabsf(point.x) || point.x >= 0) {
            return NO;
