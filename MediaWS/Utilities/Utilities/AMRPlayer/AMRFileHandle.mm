...
int EncodeWAVEFileToAMRFile(NSString *pchWAVEPath, NSString *pchAMRFileName, int nChannels, int nBitsPerSample)
{
    NSLog(@"wavpath = %@",pchWAVEPath);
    NSLog(@"pchAMRFileName = %@",pchAMRFileName);
	FILE* fpwave;
	FILE* fpamr;
	...
	fpwave = fopen([pchWAVEPath cStringUsingEncoding:NSASCIIStringEncoding], "rb");
	if (fpwave == NULL)
	{
		return 0;
	}
	...
	NSString *amrPath = [cachePath stringByAppendingPathComponent:pchAMRFileName];
	NSLog(@"amrPath=%@", amrPath);
	
    // 创建并初始化amr文件
	fpamr = fopen([amrPath cStringUsingEncoding:NSASCIIStringEncoding], "wb");
	if (fpamr == NULL)
	{
		fclose(fpwave);
		return 0;
...
void PlayAMRFile(const char* pchAMRFilePath) 
{
    ...
    audioStreamPlayer->SetStreamFormat();
    ...
    NSString *amrPath = [NSString stringWithFormat:@"%s", pchAMRFilePath];
	NSLog(@"amrPath=%@", amrPath);
    
	fpamr = fopen([amrPath cStringUsingEncoding:NSASCIIStringEncoding], "rb");
	if ( fpamr==NULL ) return;
	...
	while(1)
	{
		...
		nFrameCount++;
        usleep(15000);
        audioStreamPlayer->AddQueueBuffer(pcmFrame);
        if(jj == 10)
        {
            printf("call startQueue status: %ld\n", audioStreamPlayer->StartQueue(false));
            //        sleep(20);
        }
        jj++;
